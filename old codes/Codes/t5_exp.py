...
def run_experiment(sample_size, T):
    ...
    for bw_type in BANDWIDTH_TYPES:
        ...
        for _ in range(num_experiments):
            data = np.random.standard_t(t_df, size=(sample_size, 1)) + true_mode
...
# Save summaries
with open("results_t5.txt", "w") as f:
...
with open("mse_runs_t5.txt", "w") as f:
    for key, res in all_results.items():
        f.write(f"======== {key} ========\n")
        for bw_type in BANDWIDTH_TYPES:
            f.write(f"-- BW={bw_type} --\n")
...
with open("modes_runs_t5.txt", "w") as f:
    for key, res in all_results.items():
        f.write(f"======== {key} ========\n")
        for bw_type in BANDWIDTH_TYPES:
            f.write(f"-- BW={bw_type} --\n")
...
for config_key, config_res in all_results.items():
    ...
    plt.ylabel("RMSE")
    plt.title(f"RMSE vs Bandwidth for {config_key} (t(5))")
    plt.legend()
    plt.grid(True)
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f"rmse_vs_bandwidth_t5_{config_key}.png")
    plt.show()
