# univariate_t_mixture_dpms_sgd.py
...
from mean_shift_dp_sgd import mean_shift_dp_sgd  # DP-GEMS implementation
from merge import (
    merge_modes,
    merge_modes_dbscan,
...
def generate_univariate_t_mixture(n_samples, means, df=10):
    """
    Generate a univariate mixture of Student-t distributions.
    Returns:
      data: array of shape (n_samples, 1)
      true_modes: array of shape (k, 1)
    """
    k = len(means)
    per = n_samples // k
    data = np.vstack([
        sps.t.rvs(df, loc=m, scale=1.0, size=per).reshape(-1, 1)
        for m in means
    ])
...
def compute_mse(true_modes, est_modes):
    """
    Mean squared error between true modes and estimated modes.
    """
    return np.mean([
...
def save_text(path, text):
    """
    Create directory (if needed) and save text to file.
    """
...
def main():
    ...
    results_dir = 'SGD_results/univariate_t_mixture'
    os.makedirs(results_dir, exist_ok=True)

        # ─── Data ────────────────────────────────────────────────────────────────
    np.random.seed(42)
    data, true_modes = generate_univariate_t_mixture(n_samples, means_list, df=10)
    ...
    pdf = sum(sps.t.pdf(xs, df=10, loc=m) for m in means_list)

    plt.figure(figsize=(8, 4))
    plt.plot(xs, pdf, label="True PDF (t mix)")
