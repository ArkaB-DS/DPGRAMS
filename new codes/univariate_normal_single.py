...
def generate_univariate_normal(n_samples, mean, var):
    std = np.sqrt(var)
    data = np.random.normal(loc=mean, scale=std, size=n_samples)
    return data.reshape(-1, 1), np.array([[mean]])
...
def main(
    ...
    np.random.seed(42)
    data, true_modes = generate_univariate_normal(n_samples, mean, var)
    ...
    for run in range(n_experiments):
        ...
        # ms_modes = merge_modes_agglomerative(ms_modes_raw, n_clusters=1)
        ...
        # dp_modes = merge_modes_agglomerative(dp_modes_raw, n_clusters=1)
    ...
    save_stats("results/univariate_normal_single_stats.txt", S, R, sigma_squared, bandwidth, epsilon, delta, c_opt=c_opt)
    save_mses("results/univariate_normal_single_ms_dpms_mses.txt", ms_mses, dpms_mses)
    save_modes("results/univariate_normal_single_ms_dpms_modes.txt", ms_modes_all, dpms_modes_all)
    ...
    xs = np.linspace(mean - 5 * np.sqrt(var), mean + 5 * np.sqrt(var), 1000)

    pdf = stats.norm.pdf(xs, loc=mean, scale=np.sqrt(var))

    plt.plot(xs, pdf, label="True Distribution (Normal)", color="blue")
    ...
    plt.vlines(ms_modes.flatten(), 0, max(pdf), color="red", linestyle="-", label="MS Mode")
    plt.vlines(dp_modes.flatten(), 0, max(pdf), color="orange", linestyle="-", label="DPMS Mode")

    plt.title("Univariate Mean Shift vs DPMS (Single Normal)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/univariate_normal_single_modes_comparison.png")
    plt.show()
...
if __name__ == "__main__":
    main(
        n_samples=10000,
        mean=10.0,       # User-specified mean
        var=1.0,        # User-specified variance
        epsilon=1.0,
        delta=1e-6,
        T=20,
        p=0.1,
