...
# List of bandwidths: Silverman plus manual range
manual_bandwidths = np.round(np.arange(0.6, 1.01, 0.1), 2)  # 0.6 to 1.0
...
def run_experiment(sample_size, T):
    ...
    for bw_type in BANDWIDTH_TYPES:
        ...
        for _ in range(num_experiments):
            data = np.random.normal(loc=true_mode, scale=1.0, size=(sample_size, 1))
...
with open("modes_runs.txt", "w") as f:
    for key, res in all_results.items():
        f.write(f"======== {key} ========\n")
        for bw_type in BANDWIDTH_TYPES:
            f.write(f"-- BW={bw_type} --\n")
